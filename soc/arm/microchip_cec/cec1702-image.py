...
def private_key_to_raw_public_key(p):
	...
	return p.public_key().public_bytes(serialization.Encoding.X962, serialization.PublicFormat.UncompressedPoint)[1:]
...
def load_pem_private_key(fn):
	try:
		return serialization.load_pem_private_key(load_file(fn), password="ECPRIVKEY001", backend=backend)
	except:
		pass
	try:
		return serialization.load_pem_private_key(load_file(fn), password=os.getenv("PEM_PASSPHRASE"), backend=backend)
...
def main():
	...
	for x, req, offs in ('img0', True, 0x001000), ('img1', False, 0x081000):
		...
		g.add_argument("--%s-encrypt" % x, help="Encrypt using certificate public key (X.509 PEM)", type=lambda fn: x509.load_pem_x509_certificate(load_file(fn), backend).public_key(), metavar="PEM")
